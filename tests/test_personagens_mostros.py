...
from jogo.personagens.monstros import (
    ...
    Topera, Mico, Sucuri, ArvoreDeku, Dragao
)
from jogo.itens.item_secundario import (
    Adaga as AdagaSecundaria, Livro, Aljava, BolaDeCristal, Buckler, Escudo
)
from jogo.itens.armas import (
    Adaga, Arco_curto, Arco_longo, Botas_de_ferro, Cajado, Cajado_negro,
    Espada_curta, Espada_longa, Luvas_de_ferro, Machado
)
from jogo.itens.vestes import (
    Elmo, Amuleto, Anel, Botas, Calca, Luvas, Peitoral
)
...
class TestMonstroSortearDropsTestsAvulsos(TestCase):
    ...
    def test_item_nao_e_guardado_caso_o_inventario_do_personagem_estiver_cheio(self, randint, choice, *_):
        choice.return_value = MagicMock(tipo="Arma")
        self.monstro.sortear_drops(self.personagem)
        self.mock.assert_not_called()
    
    def test_item_e_guardado_caso_o_inventario_do_personagem_nao_estiver_cheio(self, randint, choice, *_):
        self.personagem.inventario.pop()
        choice.return_value = MagicMock(tipo="Arma")
        self.monstro.sortear_drops(self.personagem)
        self.mock.assert_called()
    
    def test_randint_e_chamado_dando_pratas_ao_personagem(self, randint, choice, *_):
        choice.return_value = MagicMock(tipo="Arma")
        self.monstro.sortear_drops(self.personagem)
...
class TestDragaoDropandoItemQuestCoracao(TestCase):
    def setUp(self):
        self.personagem = Arqueiro('nome', True)
        self.monstro = Dragao()
